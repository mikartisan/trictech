                    ...
                    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <!-- QR Code Generator -->
                                <li>
                                    <a href="qrcode.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group ">
                                    <i class="fa fa-qrcode text-xl text-gray-500 flex-shrink-0 group-hover:text-gray-900 transition duration-75" aria-hidden="true"></i>
                                    <span class="ml-3 flex-1 whitespace-nowrap">Generate QR Code</span>
                                    </a>
                                </li>
                    ...
                    <div class="pt-2 px-4">
                        <div class="mt-4 w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            <!-- <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                                <div class="flex items-center">
                                    ...
                                    <h3 class="text-base font-normal text-gray-500">Lorem Ipsum</h3>
                                    </div>
                                </div>
                            </div> -->
                            ...
                            <!-- <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-2xl sm:text-3xl leading-none font-bold text-gray-900">₱ 
                                        <?php
                                            // $income = "SELECT status, COUNT(*) as income FROM parcel WHERE status='delivered' AND email='$getUser' GROUP BY status";
                                            // $result = $conn->query($income);

                                            // if ($result->num_rows > 0) {
                                            //     while($row = $result->fetch_assoc()) {
                                            //         $parcelCount = $row["income"];
                                            //         $total = $parcelCount * 15; //Estimated Income For Example Only
                                            //         echo $total;
                                            //     }
                                            // } else {
                                            //     echo "0";
                                    ...
                                    <h3 class="text-base font-normal text-gray-500">Today's Income</h3>
                                    </div>
                        ...
                        <div class="grid grid-cols-1 2xl:grid-cols-2 xl:gap-4 my-4">
                                    ...
                                    <div class="max-h-80 overflow-y-auto"> 
                                        <table class="items-center w-full bg-transparent border-collapse">
                                            ...
                                            <tbody class="divide-y divide-gray-100">
                                                <?php 
                                                    //Display all booking transaction
                                                    $transaction = mysqli_query($conn,"SELECT * FROM booking WHERE driver_id='$unique_id' AND status != 'pending' AND status != 'accepted' AND passenger_id !='' ORDER BY id DESC");
                                                        ...
                                                        while($row = mysqli_fetch_assoc($transaction)){
                                                            ...
                                                            <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left"><?php echo $formattedDateTime = date('F j, Y g:i A', strtotime($created_at)); ?></th>
                                                                <?php
                                                                    ...
                                                                    {
                                                                        ?>
                                                                        <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap bg-red-200 text-red-600 text-left">Cancelled</th>
                                                                        <?php
                            ...
                            <div class="bg-white shadow rounded-lg p-4 mb-4 mt-4 sm:mt-0 sm:p-6 sm:mb-0 xl:p-8 ">
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-xl font-bold leading-none text-gray-900">Passenger Reviews</h3>
                                    <a href="user-reviews.php" class="text-sm font-medium text-cyan-600 hover:bg-gray-100 rounded-lg inline-flex items-center p-2">
                                    View all
                                    </a>
                                </div>
                                <div class="block w-full overflow-x-auto max-h-[450px] lg:max-h-[310px]">
                                    <table class="items-center w-full bg-transparent">
                                        <thead>
                                            <tr>
                                                <th class="px-4 bg-gray-50 text-gray-700 align-middle py-3 text-xs font-semibold text-left uppercase border-l-0 border-r-0 whitespace-nowrap">Reviews</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-100">
                                            <?php 
                                                //Display all pending deliviries
                                                $review = mysqli_query($conn,"SELECT * FROM driver_ratings WHERE driver_id = '$unique_id'  ORDER BY id DESC");
                                                                ...
                                                                <article class="pt-6 pb-6 border-b border-gray-300">
                                                                    <div class="flex items-center mb-4">
                                                                        <div class="font-medium dark:text-white">
                                                                            <p>
                                                                                <?php 
                                                                                    $maskedName = substr($name, 0, 1) . str_repeat("*", strlen($name) - 1);  // Replace with asterisks except first letter
                                                                                    echo $maskedName;
                                                                                ?> 
                                                                                <time class="block text-sm text-gray-500 dark:text-gray-400">Joined on <?php echo $getDate ? strftime("%B %d, %Y", strtotime($getDate)) : ""; ?></time></p>
                                                                        </div>
                                                                    </div>

<?php
    ...
    if(isset($_POST["submit"])) {
        if(empty($_POST['email']) || empty($_POST['password'])) {
            ...
            $email = $_POST['email']; // email or number
            $password = $_POST['password'];

            // Create a md5 hash of the password
            $hash = md5($password);

            $stmt = mysqli_query($conn,"SELECT * FROM account WHERE (email='$email' OR contact='$email') AND password='$hash'");
            ...
            if($auth){
                ...
                if ($result->num_rows > 0) {
                    ...
                    $suspension = "SELECT * FROM account WHERE suspension = 'suspended' AND email='$email' OR contact='$email'";
                    $result = $conn->query($suspension); 
                    if ($result->num_rows > 0) {
                        ...
                        if (isset($_COOKIE['driver_id'])) {
                            $driverId = $_COOKIE['driver_id'];  // Get driver_id from cookie
                            header('Location: ../passenger/tricycle/action.php?driver_id=' . $driverId);
                        } else {

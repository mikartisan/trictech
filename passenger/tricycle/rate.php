...
<body class="bg-gray-50">
    ...
    <div>
                    ...
                    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <!-- <li>
                                    <a href="../fare.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group ">
                                    <i class="fa fa-gratipay text-xl text-gray-500 flex-shrink-0 group-hover:text-gray-900 transition duration-75" aria-hidden="true"></i>
                ...
                <div class="pt-6 px-4">
                        <div class="w-full grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
                            ...
                            <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8  2xl:col-span-2">
                                <p class="text-xl font-bold">Rate Driver</p>
                                <div class="mb-3 mt-3">
                                    <div class="flex flex-row space-x-1 rating-container">
                                    <span class="star text-gray-400 text-2xl w-5 h-5">★</span>
                                    <span class="star text-gray-400 text-2xl w-5 h-5">★</span>
                                    <span class="star text-gray-400 text-2xl w-5 h-5">★</span>
                                    <span class="star text-gray-400 text-2xl w-5 h-5">★</span>
                                    <span class="star text-gray-400 text-2xl w-5 h-5">★</span>
                                </div>
                                <div class="editor mx-auto mt-8 flex flex-col text-gray-800 bg-white">
                                    ...
                                    <textarea name="comment" id="comment" class="comment bg-gray-100 sec p-3 h-60 border border-gray-300 outline-none" spellcheck="false" placeholder="Share more thoughts about the driver to help other passengers."></textarea>

                                    <!-- Buttons -->
                                    <div class="buttons flex justify-end mt-2">
                                        <a href="driver-info.php?driver_id=<?php echo $_COOKIE['driver_id'] ?? "$driver_id"; ?>" class="bg-red-400 hover:bg-red-500 text-white font-bold py-2 px-4 mr-1 rounded">
                                            Cancel
                                        </a>
                                        <button id="rateBtn" class="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded">
                                            Submit
                                        </button>
                                    </div>
                                </div>
        ...
        <script>
            ...
            // Control Star Ratings
            var star_count = 0 ; // get the star rating count
            const stars = document.querySelectorAll(".rating-container span");
            stars.forEach((star, index) => {
                star.addEventListener("click", () => {
                    stars.forEach((star) => star.classList.remove("active", "text-gray-400", "text-yellow-500"));

                    for (let i = 0; i <= index; i++) {
                        stars[i].classList.add("active", "text-yellow-500");
                    }
            ...
            $(document).ready(function() {
                $('#rateBtn').click(function() {
                    ...
                    if (title.trim() === '' || comment.trim() === '' || star_count === 0) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Star, Title and description cannot be empty!',
                        });
                        ...
                        success: function (response) {
                            ...
                            if(response.message === 'badword') {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Oops...',
                                    text: 'Your comment contains inappropriate language. Please review and try again.',
                                });
                            } else {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Rate Submitted',
                                    text: 'Thank you for rating the driver!',
                                }).then(() => {
                                    // Redirect to another page
                                    function getDriverIdCookieValue() {
                                    ...
                                    const driverId = getDriverIdCookieValue();

                                    if (driverId) {
                                        window.location.href = `driver-info.php?driver_id=${driverId}`;
                                    } else {
                                        window.location.href = '../booking.php';
                                        // Handle the missing cookie case here, e.g., display an error message or redirect to a different page
                                    }
                                });
                            }
            ...
            // Points Information
            const pointInfoLink = document.getElementById('point-info-link');
            pointInfoLink.addEventListener('click', function(event) {
                event.preventDefault();

                // Show Sweet Alert with point information
                Swal.fire({
                    title: 'Where can I use my points?',
                    text: 'You can use your points to get discounts on your next rides. Check out our Points & Rewards page for more information.',
                    icon: 'info'
                });
            });
        </script>

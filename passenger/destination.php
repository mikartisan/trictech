<?php
    ...
    $sqlUser = mysqli_query($conn,"SELECT * FROM account INNER JOIN booking_verification ON account.unique_id = booking_verification.verification_id  WHERE unique_id = '$unique_id';");
    ...
    */
    ...
    if(mysqli_num_rows($sqlFare) > 0){
        while($row = mysqli_fetch_assoc($sqlFare)){
            $getInitialFare = $row['first_kilometers'];
            $getFarePerKM = $row['succeeding_kilometers'];
...
<body class="bg-gray-50" onload="getMyLocation()">
    <div>
                            ...
                            <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                                <div class="mb-1 flex items-center justify-between">
                                    <div>
                                        ...
                                        <span class="text-base font-normal text-gray-500">Select your location and destination.</span>
                                    </div>
                                </div>
                                            ...
                                            <div class="shadow overflow-hidden sm:rounded-lg">
                                                <form method="POST">
                                                    ...
                                                    <div class="flex items-center border-2 py-2 px-2 rounded-2xl mb-4 mt-6">
                                                        ...
                                                        <input type="text" id="origin" value="" placeholder="Origin" class="pl-3 w-full outline-none border-none" />
                                                    </div>
                                                    ...
                                                    <div class="flex items-center border-2 py-3 px-2 rounded-2xl mb-4 mt-4">
                                                        ...
                                                        <input type="text" id="to" placeholder="type here or click any location on the map" class="pl-3 w-full outline-none border-none" />
                                                    </div>

                                                    <input hidden type="text" id="distance" placeholder="" class="pl-3 w-full outline-none border-none" />
                                                    <input hidden type="text" id="fare" placeholder="" class="pl-3 w-full outline-none border-none" />
                                                    <input hidden type="text" id="passenger_type" value="<?php echo $passengerType;  ?>" placeholder="" class="pl-3 w-full outline-none border-none" /> 
                                                </form>
                                                ...
                                                <div class='rounded-md text-black mb-4 mt-4 p-3'>
                                                    ...
                                                    <ul class="text-base text-[#333] space-y-2 mt-2">
                                                        <li class="flex flex-wrap gap-4">Passenger Type <span class="ml-auto"><?php echo $passengerType; ?></span></li>
        ...
        <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDBXQ709mh8Hcuv3tZzU4vhUgG2isUPPPE&loading=async&libraries=places&callback=initMap" async defer></script>

        <script>
            localStorage.removeItem('isBookingReloaded'); 
            localStorage.removeItem('isBookingCompleted'); 
            localStorage.removeItem('isBookingCancelled'); 
            /**
             * Calculate special trip estimated fare
             */
            function calculateFare(distanceInKm) {
                // Initial fare for the first 3 kilometers
                var initialFare = <?php echo $getInitialFare ?? ""; ?>;

                // Fare for each succeeding kilometer
                var farePerKm = <?php echo $getFarePerKM ?? ""; ?>;
                var passengerType = "<?php echo $passengerType ?? ""; ?>";
                var discount = <?php echo $getDiscount ?? ""; ?>;

                // Calculate fare based on the new rules:
                var fare = 0;

                if (distanceInKm <= 2) {
                    // Apply initial fare for the first 3 kilometers
                    fare = initialFare;
                } else {
                    // Apply initial fare for the first 3 kilometers
                    fare = initialFare;
                    ...
                    fare += (distanceInKm - 2) * farePerKm;
                }

                // Apply discount if passenger type is not regular
                if(passengerType !== "Regular") {
                    var discountAmount = fare * (discount / 100); // Calculate discount amount
                    fare -= discountAmount; // Subtract discount from total fare
                }
            ...
            function confirmBooking() {
                ...
                const passenger_type = document.getElementById("passenger_type").value;
                const driver_id = '<?php echo $driver_id; ?>';
                const booking_id = '<?php echo $booking_id; ?>';

                if (fare === null || fare === 0) {
                    Swal.fire({
                        title: 'Fare Calculation Required',
                        text: 'Please calculate the fare before confirming the booking.',
                        icon: 'error',
                        ...
                        if (result.isConfirmed) {
                            ...
                            if (!origin || !to ) {
                                Swal.fire({
                                    ...
                                    text: 'Please enter both the origin and destination or check the fare.'
                                });
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    ...
                                    if (response.status === 'success') {
                                        Swal.fire(
                                            'Booking Send!',
                                            ' Please wait for the driver confirmation.',
                                            'success'
                                        ).then(() => {
                                            window.location.href = `book-map.php?driver_id=` + driver_id + `&booking_id=` + booking_id;// Reload the page after confirmation
                                        });
                                    } else {
                                        Swal.fire(
                                            'Error!',

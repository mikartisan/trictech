            ...
            <div class="px-3 py-3 lg:px-5 lg:pl-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center justify-start">
                        <a href="book-map.php?driver_id=<?php echo $driver_id; ?>&booking_id=<?php echo $booking_id; ?>" class="text-md font-bold text-white bg-blue-500 rounded-full px-4 py-1.5 hover:bg-blue-600">Back</a>
                    </div>
                </div>
            </div>
        </nav>
        <div class="flex pt-16" style="margin-bottom: 80px;"> <!-- Adjust the margin-bottom value as needed -->
            <div id="main-content" class="h-full w-full bg-gray-50 relative overflow-y-auto lg:ml-12">
                <main class="p-5">
                    ...
                    <?php 
                        $getChat = "SELECT * FROM chats INNER JOIN account ON account.unique_id = chats.driver_id WHERE booking_id = ? ORDER BY created_at ASC";
                        // Prepare statement
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                        ...
                                        <div class="flex items-start gap-2.5 mt-5">
                                            ...
                                            <div class="flex flex-col gap-1 w-full max-w-[320px]">
                                                <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                                    <span class="text-sm font-semibold text-gray-900"><?php echo $chatName; ?></span>
                                                    <span class="text-sm font-normal text-gray-500"><?php echo date('g:i a', strtotime($timeSent)); ?></span>
                                                </div>
                                                <?php
                                ...
                                if ($seenClass == '') {
                                    $updateSeen = "UPDATE chats SET seen = 1 WHERE booking_id = ? AND sender = 'driver'";

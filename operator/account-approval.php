<?php
    ...
    if(isset($_POST["verify"])) {
        ...
        $sql = "UPDATE account SET status='activated' WHERE id='$id'";
        mysqli_query($conn, $sql);  

        // Get the name of approved account
        $apprName = mysqli_query($conn,"SELECT * FROM account WHERE id = '$id';");
        if(mysqli_num_rows($apprName) > 0){
            while($row = mysqli_fetch_assoc($apprName)){
                $apprFName = $row["fname"] ?? " ";
                $apprLName = $row["lname"] ?? " ";
                $apprEmail = $row["email"] ?? " ";
            }
        ...
        if (mysqli_affected_rows($conn) > 0) {
            ...
            $subject = 'TricTech: Account Approved';
            $message = 'Your account has been approved by the your operator.' ;
            $headers = 'From: trictech@gmail.com';

            if (mail($apprEmail, $subject, $message, $headers)) {
                 // Saved Activity Log
                // $activity = "Rider Account : ". $apprFName ." ". $apprLName. " Approved by ". $getFName ." ". $getLName;
            ...
            $send_data['message'] = "Congratulations! Your account has been approved by your operator. You can now access and use the system.\n\n";     
            $send_data['message'] .= 'Login your account through this link: https://sme.absierra.com/';
        ...
        $userID = $_POST["decline"];

        // Get the name of approved account
        $apprName = mysqli_query($conn,"SELECT * FROM account WHERE id = '$userID';");
        if(mysqli_num_rows($apprName) > 0){
            while($row = mysqli_fetch_assoc($apprName)){
                $apprFName = $row["fname"] ?? " ";
                $apprEmail = $row["email"] ?? " ";
        ...
        if (mysqli_affected_rows($conn) > 0) {
            ...
            // Code matches, retrieve the email
            $subject = 'TricTech: Account Declined';
            $message = 'Your account has been declined by the operator.' ;
            $headers = 'From: trichtech@gmail.com';

            if (mail($apprEmail, $subject, $message, $headers)) {
                // Saved Activity Log
                // $activity = "Rider Account : ". $apprFName ." ". $apprLName. " Declined by ". $getFName ." ". $getLName;
                // $log = "INSERT INTO admin_log (fname, lname, position, activity, status, date)VALUES ('$getFName', '$getLName', '$getPosition', '$activity', 'OK', '$datetime')";
                // $pdo->exec($log);
                
                $success = true;
                $notif = "Account Declined!";
            } else {
                $error = true;
                    ...
                    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <!-- Ratings -->
                                <li>
                                    <a href="dashboard.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group ">
                                    ...
                                    <span class="ml-3 flex-1 whitespace-nowrap">Driver Ratings</span>
                                    </a>
                                </li>
                                ...
                                <!-- Account Approval -->
                                <li>
                                    <a href="account-approval.php" class="text-base text-gray-900 font-normal rounded-lg bg-gray-100 flex items-center p-2 group ">
                                    <i class="fa fa-check text-xl text-blue-600 flex-shrink-0 group-hover:text-gray-900 transition duration-75" aria-hidden="true"></i>
                                    <span class="ml-3 flex-1 text-blue-600 whitespace-nowrap">Account Approval</span>
                                    </a>
                                </li>
                            ...
                            <div class="mb-4 border-b border-gray-200 ">
                                <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="myTab" data-tabs-toggle="#myTabContent" role="tablist">
                                    <li class="mr-2" role="presentation">
                                        <button class="inline-block p-4 rounded-t-lg border-b-2 border-transparent hover:text-gray-600 id="dashboard-tab" data-tabs-target="#approval" type="button" role="tab" aria-controls="dashboard" aria-selected="false">Account Approval 
                                            <span class="ml-2">
                                                <?php
                                                    try {
                                                        $query = "
                                                            ...
                                                            INNER JOIN account a ON od.driver_id = a.unique_id
                                                            WHERE od.operator_id = '$unique_id' AND a.status = 'pending'
                                                        ";
                                ...
                                <table id="table_approval" class="display" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                        <?php 
                                            //Display all account
                                            $query = "
                                                SELECT a.*, od.driver_id FROM operator_drivers od
                                                INNER JOIN account a ON od.driver_id = a.unique_id
                                                WHERE od.operator_id = '$unique_id' AND a.status = 'pending'
                                            ";
                                            $account = mysqli_query($conn, $query);
                                            ...
                                            if(mysqli_num_rows($account) > 0){
                                                while($row = mysqli_fetch_assoc($account)){
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <?php echo ($position === 'Rider') ? 'Driver' : $position; ?>
                                                        </td>
                                                        <td> <!-- Visit Account -->
                                                            <form method="POST">
                                                                <button formaction="view-profile.php?viewProfile=<?php echo $id; ?>" name="viewProfile" type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
                                                                    <i class="fa fa-user text-sm" aria-hidden="true"></i>
                                                                    View
                                                                </button>
                                                                <button name="verify" value="<?php echo $id; ?>" type="submit" class="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
                                                                    <i class="fa fa-check text-sm" aria-hidden="true"></i>
                                                                    Verify
                                                                </button>
                                                                <button name="decline" value="<?php echo $id; ?>" type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">

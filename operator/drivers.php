<?php
    ...
    if(isset($_POST["verify"])) {
        ...
        if (mysqli_affected_rows($conn) > 0) {
            ...
            $message = 'Your account has been approved by your operator.' ;
                    ...
                    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <!-- Driver Account -->
                                <li>
                                    <a href="drivers.php" class="text-base text-gray-900 font-normal rounded-lg bg-gray-100 flex items-center p-2 group ">
                                    <i class="fa fa-male text-xl text-blue-600 flex-shrink-0 group-hover:text-gray-900 transition duration-75" aria-hidden="true"></i>
                                    <span class="ml-3 flex-1 text-blue-600 whitespace-nowrap">Your Drivers</span>
                                    </a>
                                </li>
                                            ...
                                            <?php
                                                try {
                                                    $query = "
                                                        ...
                                                        WHERE od.operator_id = '$unique_id' AND a.status = 'activated'
                                                    ";
                                    ...
                                    <?php
                                        ...
                                        if (mysqli_num_rows($account) > 0) {
                                            while ($row = mysqli_fetch_assoc($account)) {
                                                // Extract relevant information from the $row array
                                                $driver_fname = $row['fname'];
                                                $driver_lname = $row['lname'];
                                                $driver_email = $row['email'];
                                                $driver_position = $row['position'];

                                                // Display the information in the <td> elements
                                                ?>
                                                <tr>
                                                    <td scope="row"><?php echo $driver_fname . " " . $driver_lname; ?></td>
                                                    <td><?php echo $driver_email; ?></td>
                                                    <td><?php echo ($driver_position === 'Rider') ? 'Driver' : $driver_position; ?></td>
                                                    <td>
                                                        <form method="GET">
                                                            <!-- Visit Account -->
                                                            <button formaction="view-profile.php" name="viewProfile" value="<?php echo $row['id']; ?>" type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">

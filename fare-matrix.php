<?php
    ...
    // Insert New Fare
    if(isset($_POST["addFareSubmit"])) {
        $origin = $_POST["origin"]; 
        $destination = $_POST["destination"]; 
        $fare = $_POST["fare"]; 

        $sql = "SELECT * FROM fare_matrix WHERE destination='$destination'";
        $result = mysqli_query($conn, $sql);
        $num = mysqli_num_rows($result); 
        
        // Check if user already registered
        if($num == 0) {
            $sql = "INSERT INTO `fare_matrix` (`origin`, `destination`, `fare`) VALUES ('$origin', '$destination', '$fare')";
            $result = mysqli_query($conn, $sql);
            if ($result) {
                $statNotif = true;
                ...
                //Saved Activity Log
                $activity = "The fare of ". $fare ." pesos from origin: ". $origin ." to destination: ". $destination ." has been added";
                $log = "INSERT INTO user_log (activity_id, fname, lname, position, activity, status, date)VALUES ('$unique_id', '$getFirstName', '$getLastName', '$getPosition', '$activity', 'OK', '$datetime')";
                $pdo->exec($log);
            ...
            $statNotif = true;
            $notif = "Destination Already Exist."; 
        } 
    }
...
<body class="bg-gray-50">
    <div>
                        ...
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <?php if($getType === 'municipality') : ?>
                                    <li>
                                        <a href="fare-matrix.php" class="text-base text-gray-900 font-normal rounded-lg bg-gray-100 flex items-center p-2 group ">
                                        <i class="fa fa-map text-xl text-blue-600 flex-shrink-0 transition duration-75" aria-hidden="true"></i>
                                        <span class="ml-3 flex-1 whitespace-nowrap text-blue-600">Fare Matrix</span>
                                        </a>
                                    </li>
                                <?php endif; ?>
                        ...
                        <div class="w-full grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
                            <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
                                <div class="mb-1 flex items-center justify-between">
                                    <div>
                                        <h3 class="text-xl font-bold text-gray-900 mb-2">Fare Matrix</h3>
                                        <span class="text-base font-normal text-gray-500">Add, Edit and Delete fare matrix table here.</span>
                                    </div>
                                </div>
                                    ...
                                    <div class="overflow-x-auto rounded-lg">
                                        <div class="align-middle inline-block min-w-full">
                                            <div class="flex justify-between mb-3 mt-5">
                                                <button
                                                    type="button"
                                                    class="text-md font-bold text-white bg-blue-500 rounded-full px-5 py-3 w-full hover:bg-blue-600"
                                                            ...
                                                            <div class="relative flex-auto p-4" data-te-modal-body-ref>
                                                                <form method="POST">
                                                                    <div class="modal-body relative p-4">
                                                                        <div class="mb-3">
                                                                            <label class="block text-gray-700 text-sm font-bold mb-2" for="exampleSelect">
                                                                                Place of Origin
                                                                            </label>
                                                                            <select name="origin" class="appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" id="exampleSelect">
                                                                                <option value="Town Proper">Town Proper</option>
                                                                                <option value="Eeast Cam Tech">East Cam Tech</option>
                                                                        ...
                                                                        <div class="mb-3">
                                                                            <label for="lname" class="block mb-2 text-sm font-medium text-gray-900 ">Place of Destination (Vice Versa</label>
                                                                            <input type="text" name="destination" id="destination" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Destination" required>
                                                                        </div>
                                                                        <div class="mb-3">
                                                                            <label for="fare" class="block mb-2 text-sm font-medium text-gray-900 ">New Fare (Per Head)</label>
                                                                            <input type="text" name="fare" id="fare" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="0.00" required>
                                                                        </div>
                                                                    </div>
                                                                    <div
                                                                        ...
                                                                        <button type="submit" name="addFareSubmit"
                                ...
                                <div class="mb-1 mt-4 flex items-center justify-between">
                                    <div>
                                        <h3 class="text-xl font-bold text-gray-900 mb-2">Special Trip</h3>
                                        <span class="text-base font-normal text-gray-500">Add and edit fares for special trips.</span>
                                    </div>
                                </div>
                                <div class="mb-4 mt-5">
                                    <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                                        Regular Fare (First 3 Kilometers)
                                    </label>
                                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="firstKilometer"  value="<?php echo $first_kilometers; ?>" type="number" placeholder="₱ 0.00">
                                </div>
                                <div class="mb-4 mt-5">
                                    <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
                                        Fare For every succeeding kilometers
                                    </label>
                                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="succeedingKilometer"  value="<?php echo $succeeding_kilometers; ?>" type="number" placeholder="₱ 0.00">
                                </div>

                                <!-- Discounts -->
                                <div class="mb-1 mt-4 flex items-center justify-between">
                                    <div>
                                        <h3 class="text-xl font-bold text-gray-900 mb-2">Discount</h3>
                                        <span class="text-base font-normal text-gray-500">For Students, Senior Citizen, Persons with Disability (PWD)</span>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4 mt-5">
                                    ...
                                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="discount" value="<?php echo $discount; ?>" type="number" placeholder="Discount (%)">
                                </div>
                                <div class="mt-5">
                                    <button
                                        type="button"
                                        onclick="saveFareAndDiscount()"
                                        class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 w-full border border-green-700 rounded">
                            ...
                            <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8  2xl:col-span-2">
                                <div class="">
                                    <table id="table_category" class="display" cellspacing="0">
                                        <thead>
                                            <tr>
                                                <th scope="col" class="py-3 px-6">Place of Origin</th>
                                                <th scope="col" class="py-3 px-6">Place of Destination (Vice Versa)</th>
                                                <th scope="col" class="py-3 px-6">Fare (Per Head)</th>
                                                <th scope="col" class="py-3 px-6">Student / Senior / PWD</th>
                                        ...
                                        <tbody>
                                            <?php 
                                                //Display all category
                                                $sql = mysqli_query($conn,"SELECT * FROM fare_matrix");
                                                if(mysqli_num_rows($sql) > 0){
                                                    while($row = mysqli_fetch_assoc($sql)){
                                                        ...
                                                        ?>
                                                        <tr>
                                                            <td scope="row"><b><?php echo $origin; ?></b></td>
                                                            <td scope="row">
                                                            ...
                                                            <td scope="row">
                                                                ₱
                                                                <?php 
                                                                    $percent = ($discount / 100) * $fare;
                                                                    $final_fare = $fare - $percent;
                                                                    echo intval($final_fare);
                                                                ?>
                                                            </td>
                                                            <td>
                                                                <!-- <button onclick="showEditModal(<?php echo $id; ?>)" type="button" class="bg-blue-400 hover:bg-blue-500 text-white font-bold w-24 text-xs xl:text-xs py-2 px-3 rounded hover:shadow-lg">
                                                                    <i class="fa fa-pen text-sm" aria-hidden="true"></i>
                                                                    Edit
                                                                </button> -->
                                                                <button onclick="deleteFare(<?php echo $id; ?>)" type="button" class="bg-red-400 hover:bg-red-500 text-white font-bold w-24 text-xs xl:text-xs py-2 px-3 rounded hover:shadow-lg">
                                                                    <i class="fa fa-close text-sm" aria-hidden="true"></i>
                                                                    Delete
                                                                </button>
                                                            </td>
        ...
        <script>
            ...
            function deleteFare(deleteID) {
                Swal.fire({
                    title: 'Delete',
                    text: "Are you sure you want to delete the fare?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: 'delete-fare.php',
                            method: 'POST',
                            data: { deleteID: deleteID }, // Pass the id to the server
                            ...
                            success: function(response) {
                                ...
                                if (response.status === 'success') {
                                    Swal.fire(
                                        ...
                                        window.location.href = `fare-matrix.php`; // Reload the page after confirmation
                                    });
                                } else {
                                    Swal.fire(
                                        'Error!',
                                        'Failed to delete fare. Please try again.',
                                        'error'
                                    );
                                }
                            }
            ...
            function showEditModal(id) {
                ...
                var htmlContent = `
                    <div class="modal-body relative p-4">
                        ...
                        <div class="mb-3">
                            <label for="lname" class="block mb-2 text-sm font-medium text-gray-900 ">Place of Destination (Vice Versa)
        </label>
                            <input type="text" name="destination" id="destination" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Destination" required>
                        </div>
                        <div class="mb-3">
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.status === 'success') {
                                    ...
                                    Swal.fire(
                                        'Error!',
                                        'Failed to edit fare. Please try again.',
                                        'error'
                                    );
                                }
                            }
            ...
            function saveFareAndDiscount() {
                var firstKilometer = $('#firstKilometer').val();
                var succeedingKilometer = $('#succeedingKilometer').val();
                var discount = $('#discount').val()

                Swal.fire({
                    title: 'Save Fare and Discount',
                    text: "Are you sure you want to save the fare and discount?",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.isConfirmed) {
                        // Now, send the SOS with the retrieved address
                        $.ajax({
                            url: 'save-fare-discount.php',
                            method: 'POST',
                            data: { 
                                first_km : firstKilometer,
                                succ_km : succeedingKilometer,
                                discount : discount,
                            ...
                            success: function(response) {
                                ...
                                if (response.status === 'success') {
                                    Swal.fire(
                                        'Fare Saved',
                                        'Fare/Discount Saved Successfully!',
                                        'success'
                                    ).then(() => {

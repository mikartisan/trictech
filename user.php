<?php
    ...
    if(isset($_POST["addUserSubmit"])) {
        ...
        $lname = $_POST["lname"]; 
        $email = $_POST["email"]; 
        $emailConfirm = $_POST["con-email"]; 
        $password = $_POST["password"]; 

        // Generate a unique code
        $uniqueId = substr(uniqid(), 7, 8);
        $hash = md5($uniqueId);
        ...
        if($num == 0) {
            if($email==$emailConfirm) {
                $sql = "INSERT INTO `account` ( `fname`, `lname`, `email`, `password`, `position`, `type`, `status`, `date`) VALUES ('$fname', '$lname', '$email', '$hash', 'Administrator', 'admin', 'activated', '$datetime')";
                $result = mysqli_query($conn, $sql);
                if ($result) {

                    // Code matches, retrieve the email
                    $subject = 'Administrator Account for Courier System';
                    $message = 'We are delighted to provide you with your account details for our system. Please find below the necessary information:' . "\r\n\r\n" .
                                'Username: ' . $email . "\r\n" .
                                'Your temporary password is: ' . $uniqueId . "\r\n\r\n" .
                                'Please ensure that you keep your account details confidential and do not share them with anyone. If you have any questions or need further assistance, please feel free to contact our support team.' . "\r\n\r\n" .
                                'Thank you for being a part of our system!';
                    $headers = 'From: jmbdeocampo@bpsu.edu.ph';

                    if (mail($email, $subject, $message, $headers)) {
                        $success = true;
                        $notif = "Account sent to Email!";

                        //Saved Activity Log
                        $activity = $getPosition ." added ".$fname." ".$lname." as new Administrator.";
                        ...
                        $error = true;
                        $notif = "Email could not be sent.". $email;
                    }
    ...
    <div>
            ...
            <div class="px-3 py-3 lg:px-5 lg:pl-3">
                <div class="flex items-center justify-between">
                    <div class="flex items-center justify-start">
                        ...
                        <button id="toggleSidebarMobileSearch" aria-expanded="true" aria-controls="sidebar" class="lg:hidden mr-2 text-gray-600 hover:text-gray-900 cursor-pointer p-2 hover:bg-gray-100 focus:bg-gray-100 focus:ring-2 focus:ring-gray-100 rounded">
                    ...
                    <div class="flex items-center">
                        <div class="hidden lg:block">
                            <a href="#" id="dropdownDefault" data-dropdown-toggle="dropdown" class="sm:inline-flex ml-5 text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3">
                            ...
                            <div id="dropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow ">
                                ...
                                <ul class="py-1 text-sm text-gray-700 " aria-labelledby="dropdownDefault">
                                    <li>
                                        <a href="profile.php" class="block py-2 px-4 hover:bg-gray-100  ">Profile</a>
                                    </li>
                                    <li>
                                        <a href="./auth/logout.php" class="block py-2 px-4 hover:bg-gray-100  ">Sign out</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div class="flex-1 px-3 bg-white divide-y space-y-1">
                            <ul class="space-y-2 pb-2">
                                ...
                                <!-- User -->
                                <?php if($getType === 'admin') : ?>
                                    <a href="user.php" class="text-base text-gray-900 font-normal rounded-lg bg-gray-100 flex items-center p-2 group ">
                                    <i class="fa fa-users text-xl text-blue-600 flex-shrink-0 group-hover:text-gray-900 transition duration-75" aria-hidden="true"></i>
                                    <span class="ml-3 flex-1 whitespace-nowrap text-blue-600">Users</span>
                                    </a>
                                <?php endif; ?>
            ...
            <div id="main-content" class="h-full w-full bg-gray-50 relative overflow-y-auto lg:ml-64">
                <main>
                    <div class="pt-6 px-4 bg-white-400">
                        <div class="bg-white shadow rounded-lg p-4 mb-4 sm:p-6 sm:mb-0 xl:p-8 ">
                            <div class="mb-4 border-b border-gray-200 ">
                                <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="myTab" data-tabs-toggle="#myTabContent" role="tablist">
                                    <li class="mr-2" role="presentation">
                                        <button class="inline-block p-4 rounded-t-lg border-b-2" id="profile-tab" data-tabs-target="#profile" type="button" role="tab" aria-controls="profile" aria-selected="false">Admin Account
                                            <span class="ml-2">
                                                <?php  
                                                    try {
                                                        // Prepare the SQL query to count the total rows in the table
                                                        $query = "SELECT COUNT(*) as totalRows FROM account WHERE type='admin' and status='activated'";
                                    ...
                                    <!-- <li class="mr-2" role="presentation">
                                        <button class="inline-block p-4 rounded-t-lg border-b-2 border-transparent hover:text-gray-600 id="dashboard-tab" data-tabs-target="#dashboard" type="button" role="tab" aria-controls="dashboard" aria-selected="false">Driver Account
                                            <span class="ml-2">
                                                <?php  
                                                    try {
                                                        // Prepare the SQL query to count the total rows in the table
                                                        $query = "SELECT COUNT(*) as totalRows FROM account WHERE type='rider' and status='activated'";
                                                        $statement = $pdo->prepare($query);
                                                        $statement->execute();
                                                        $row = $statement->fetch(PDO::FETCH_ASSOC);
                            ...
                            <div id="adminAcountTab">
                                <div class="" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                                    <div class="mb-6">
                                        <!-- Add Admin Button -->
                                        <button
                                            type="button"
                                            class="inline-block px-6 py-2.5 bg-blue-500 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-600 hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-600 active:shadow-lg transition duration-150 ease-in-out"
                                            data-te-toggle="modal"
                                            data-te-target="#modalAdd"
                                            data-te-ripple-init
                                        ...
                                        <a href="./function/export-user.php" target="_blank">
                                            <button class="inline-block px-6 py-2.5 bg-red-400 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg--700 active:shadow-lg transition duration-150 ease-in-out">
                                                Export User
                                            </button>
                                        </a>
                                                        ...
                                                        <form method="POST">
                                                            <div class="modal-body relative p-4">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="input" name="fname" id="fname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="ex. Juan" required>
                                                                </div>
                                                                <div class="mb-3">
                                                                    <label for="lname" class="block mb-2 text-sm font-medium text-gray-900 ">Last Name</label>
                                                                    <input type="input" name="lname" id="lname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="ex. Dela Cruz" required>
                                                                </div>
                                                                <div class="mb-3">
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="con-email" class="block mb-2 text-sm font-medium text-gray-900 ">Confirm Email</label>
                                                                    <input type="email" name="con-email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="name@gmail.com" required>
                                                                </div>
                                                                <!-- <div>
                                                                    <label for="password" class="block mb-2 text-sm font-medium text-gray-900 ">Temporary Password</label>
                                    ...
                                    <table id="table_admin" class="display" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                                <?php 
                                                    //Display all account
                                                    $account = mysqli_query($conn,"SELECT * FROM account WHERE type = 'admin' and status='activated'");
                                                    if(mysqli_num_rows($account) > 0){
                                                        while($row = mysqli_fetch_assoc($account)){
                                                            extract($row);
                                                            ?>
                                                            <tr>
                                                                <td scope="row"><?php echo $fname ." ". $lname; ?></td>
                                                                <td><?php echo $email; ?></td>
                                                                <td><?php echo $position; ?></td>
                                                                <td>
                                                                    <form method="GET">
                                                                        <!-- Visit Account -->
                                                                        <button formaction="view-profile.php" name="viewProfile" value="<?php echo $unique_id; ?>" type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
                                ...
                                <div class="hidden p-4 bg-gray-50 rounded-lg " id="dashboard" role="tabpanel" aria-labelledby="dashboard-tab">
                                    <table id="table_rider" class="display" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                                <th scope="col" class="py-3 px-6">Name</th>
                                                <th scope="col" class="py-3 px-6">Email</th>
                                        ...
                                        <tbody>
                                            <?php 
                                                //Display all account
                                                $account = mysqli_query($conn,"SELECT * FROM account WHERE type = 'rider' and status='activated'");
                                                if(mysqli_num_rows($account) > 0){
                                                    while($row = mysqli_fetch_assoc($account)){
                                                        extract($row);
                                                        ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <form method="GET">
                                                                    <!-- Visit Account -->
                                                                    <button formaction="view-profile.php" name="viewProfile" value="<?php echo $id; ?>" type="submit" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-3 rounded hover:shadow-lg focus:bg-blue-600 focus:shadow-lg focus:outline-none focus:ring-0 transition duration-150 ease-in-out">
                                                                        <i class="fa fa-user text-sm" aria-hidden="true"></i>
                                                                        View
                                                                    </button>
                                                                </form>
                                ...
                                <!-- <div class="hidden p-4 bg-gray-50 rounded-lg " id="approval" role="tabpanel" aria-labelledby="dashboard-tab">
                                    <table id="table_approval" class="display" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
                                                <th scope="col" class="py-3 px-6">Name</th>
                                                <th scope="col" class="py-3 px-6">Email</th>
        ...
        <script>
            $(document).ready(function () {
                ...
                // Listen for the tab shown event using Flowite API
                Flowite.onTabShown(function(tabId) {
                    $('#table_rider').DataTable().columns.adjust().draw();
                });
